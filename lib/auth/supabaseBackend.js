...
async function revalidateEmployee(userId, email) {
  // Without a live directory there is nothing to check against. matchByEmail
  // returns null (not an error) when Buildium is not live, and treating that
  // as "not staff" demoted a real employee on a preview deployment that had
  // no BUILDIUM_LIVE set.
  if (!isBuildiumLive()) return;
  let m;
