...
// Delete the account for good. Clears the Supabase browser session the same way
// signOut does, or the next visit tries to re-authenticate as a user that no
// longer exists and lands on an error instead of the login screen.
export async function deleteAccount() {
  const res = await fetch("/api/auth/account", { method: "DELETE" });
  const data = await res.json().catch(() => ({}));
  if (!res.ok) return { error: data.error || `Couldn't delete the account (${res.status})` };
  if (liveMode) {
    try {
      const { getBrowserSupabase } = await import("./supabase");
      await getBrowserSupabase().auth.signOut();
...
// The one-time code from the same email as the link. Inside the native app
// the link opens in the phone's browser, not the app, so the code is the only
// way to finish signing in without leaving it. Verified on the server, which
// then issues the ordinary session cookie.
export const verifyCode = (email, code) => post("/api/auth/verify-code", { email, code });
...
export async function completeBrowserSignIn() {
  ...
  // Implicit-flow returns (and errors) arrive in the hash, not the query string.
  ...
  if (code) {
    ...
    // The browser client is configured flowType:"pkce", and in that mode
    // detectSessionInUrl only looks for ?code= — it ignores hash tokens
    // completely. Links minted server-side (admin generate_link, used to let the
    // office hand someone a way in) arrive exactly that way, so a perfectly
    // valid session would otherwise sit unread in the address bar and the user
    // would be told to request a new link.
