...
import { isBuildiumWriteEnabled, isBuildiumWorkOrderWriteEnabled } from "../env.js";
import { beginWrite, finishWrite, writesHalted, currentWriteActor } from "./writeLog.js";
...
async function pageAll(path, { cap = 10, limit = 100, query = {} } = {}) {
  const out = [];
  for (let i = 0; i < cap; i++) {
    const page = await buildiumRequest(path, { query: { ...query, limit, offset: i * limit } });
...
async function tasksCached() {
  ...
  // Newest first. Unordered, Buildium pages oldest-first (as it does for
  // /workorders), so past 1,200 tasks the cap would drop the newest tickets,
  // which are exactly the open ones.
  c.data = await pageAll("/tasks", { cap: 12, query: { orderby: "Id desc" } });
  c.at = Date.now();
  return c.data;
}
...
// The rental properties themselves, by id. indexesCached() keeps only the names,
// and the owner balance needs each property's Reserve — the float that must stay
// in the account and therefore is not the owner's to draw.
const RENTALS_TTL_MS = 10 * 60 * 1000;
async function rentalsCached() {
  const c = (globalThis.__flRentals ||= { at: 0, byId: null });
  if (c.byId && Date.now() - c.at < RENTALS_TTL_MS) return c.byId;
  const rentals = await pageAll("/rentals", { cap: 5 });
  c.byId = new Map(rentals.map((r) => [r.Id, r]));
  c.at = Date.now();
  return c.byId;
}
...
export const realBuildium = {
  ...
  // What an owner can actually draw today.
  //
  // Buildium has no rental-owner balance endpoint: /rentals/owners/outstandingbalances
  // and /rentals/outstandingbalances are both 404, and the outstandingbalances
  // family only covers leases and association ownership accounts. So the figure
  // is assembled from the general ledger, filtered to this owner:
  //
  //   held      the balances on the accounts Buildium itself flags IsBankAccount
  //             and does not exclude from cash, on a CASH basis as of today.
  //             That is money actually in hand, not rent that has been invoiced.
  //   reserve   the float each property must keep. 122 of the 306 properties
  //             carry one, usually $300 — quietly dropping it would tell an
  //             owner they can draw money the office is required to hold back.
  //   available held − reserve, and it can legitimately be negative when a
  //             property has dipped below its float.
  //
  // Only the accounts are returned alongside, not the whole trial balance: the
  // twelve-row income/expense breakdown is an accounting document, and this is a
  // phone screen answering "what am I owed".
  async ownerBalance(ownerId, propertyIds = []) {
    const oid = toId(ownerId);
    if (oid == null) return null;

    // The broker's own date, not UTC. toISOString() rolls over at 7pm Eastern,
    // so an owner checking their balance in the evening was told it was "as of"
    // tomorrow — which reads like a mistake on a money figure.
    const asOf = new Intl.DateTimeFormat("en-CA", {
      timeZone: "America/New_York", year: "numeric", month: "2-digit", day: "2-digit",
    }).format(new Date());
    let rows;
    try {
      rows = await buildiumRequest("/glaccounts/balances", {
        query: { entitytype: "RentalOwner", entityid: oid, accountingbasis: "Cash", asofdate: asOf },
      });
    } catch {
      // A balance we cannot stand behind is not shown at all.
      return null;
    }
    if (!Array.isArray(rows)) return null;

    const cashRows = rows.filter(
      (r) => r.GLAccount?.IsBankAccount && !r.GLAccount?.ExcludeFromCashBalances
    );
    const held = cashRows.reduce((sum, r) => sum + (Number(r.TotalBalance) || 0), 0);

    // Reserves come from the properties themselves. Falls back to the owner's
    // own list when the rentals index hasn't been built on this instance.
    let reserve = 0;
    let counted = 0;
    try {
      const rentals = await rentalsCached();
      for (const pid of propertyIds) {
        const p = rentals.get(pid);
        if (!p) continue;
        const r = Number(p.Reserve) || 0;
        if (r <= 0) continue;
        // Only properties that actually carry one are counted. One owner here
        // holds 21 properties and a single $300 reserve; saying "less reserve ·
        // 21 properties" next to $300 invites the wrong arithmetic.
        counted++;
        reserve += r;
      }
    } catch { /* a missing reserve must not take the balance down with it */ }
    ...
    return {
      ...
      // Named so the owner can see where it comes from rather than being handed
      // a bare number.
      // Only the accounts actually holding something. This owner's money sits in
      // two of six trust accounts; listing the four empty ones buries the two
      // that answer the question.
      accounts: cashRows
        .map((r) => ({ name: r.GLAccount?.Name || "Account", balance: Number(r.TotalBalance) || 0 }))
        .filter((a) => a.balance !== 0)
        .sort((a, b) => b.balance - a.balance),
      reserveCountedOn: counted,
    };
  },
  ...
  // opts.preview forces the payload to be returned instead of sent, whatever the
  // environment says. It is a SECOND argument, deliberately: the API routes pass
  // a body parsed from JSON as `input`, so a field inside it could be set by the
  // browser — and a caller who could set "preview" could make a write silently
  // not happen while the app reported success.
  async createOrder(input, opts = {}) {
    ...
    const payload = {
      ...
      // Always New. Urgency rides on Priority; TaskStatus is about whether anyone
      // has picked the job up, and nobody has picked up a request filed one second
      // ago. This is also the clamp that stops a crafted body creating an
      // already-Completed ticket — a request nobody will ever look at.
      TaskStatus: "New",
    ...
    if (opts.preview || !writesEnabled()) return dryRun("POST", "/tasks/residentrequests", payload);

    const created = await sendWrite({ method: "POST", path: "/tasks/residentrequests", payload });
    invalidateTaskCaches();
    const idx = await indexesCached();
    return mapTaskToOrder(created, idx);
  },

  async updateOrder(id, patch, opts = {}) {
    ...
    let assignment = null;
    if (patch.vendorId != null) assignment = await realBuildium.assignVendor(taskId, patch.vendorId, patch, opts);
    ...
    if (opts.preview || !writesEnabled()) {
      const preview = dryRun("PUT", `${type.path}/${taskId}`, payload);
      return assignment ? { ...preview, assignment } : preview;
    }

    const updated = await sendWrite({
      method: "PUT", path: `${type.path}/${taskId}`, payload, taskId: Number(taskId),
      // Stored so a wrong update can be reversed, not just described.
      before: current,
    });
    invalidateTaskCaches();
    const idx = await indexesCached();
  ...
  async assignVendor(taskId, vendorId, patch = {}, opts = {}) {
    ...
    const existingId = await findWorkOrderForTask(tid);
    ...
    if (existingId != null) {
      ...
      if (opts.preview || !workOrderWritesEnabled()) return dryRun("PUT", `/workorders/${existingId}`, payload);
      const updated = await sendWrite({
        method: "PUT", path: `/workorders/${existingId}`, payload, taskId: tid, before: wo,
      });
      invalidateTaskCaches();
    ...
    if (opts.preview || !workOrderWritesEnabled()) return dryRun("POST", "/workorders", payload);
    const created = await sendWrite({ method: "POST", path: "/workorders", payload, taskId: tid });
    invalidateTaskCaches();
...
// ── Write plumbing ────────────────────────────────────────────────────────────
// Delegated to lib/env rather than reading the variable directly, so a write
// cannot happen while the app is in mock mode. These used to read
// process.env.BUILDIUM_WRITES on their own, which meant callers that import this
// module directly — the diagnostic route does — had neither the isBuildiumLive
// check nor the Proxy gate in lib/buildium/index.js standing between them and
// the broker's account.
export function writesEnabled() {
  return isBuildiumWriteEnabled();
}
...
export function workOrderWritesEnabled() {
  return isBuildiumWorkOrderWriteEnabled();
}
...
// Does this task already have a work order?
//
// /workorders cannot be filtered by task. taskid, taskIds and TaskId are all
// accepted and silently ignored — each returns the same unfiltered page — so the
// only way to know is to look.
//
// The display index covers the newest 600, which today holds a work order for
// every one of the newest 100 tasks. "Today" is not a guarantee: the account has
// 2,395 work orders and grows, and being wrong here means raising a SECOND work
// order against a job that already has one, in the broker's live account, and
// possibly emailing a contractor about it.
//
// So a miss falls back to a full scan before concluding there is none. That is
// ~24 requests, it is cached, and it only runs on the path that is about to
// create something.
const WORKORDER_SCAN_TTL_MS = 5 * 60 * 1000;
async function findWorkOrderForTask(taskId) {
  const hit = (await workOrdersByTask()).get(taskId);
  if (hit) return hit.workOrderId;

  const c = (globalThis.__flWorkOrderScan ||= { at: 0, byTask: null });
  if (!c.byTask || Date.now() - c.at >= WORKORDER_SCAN_TTL_MS) {
    const byTask = new Map();
    for (let i = 0; i < 40; i++) {
      const page = await buildiumRequest("/workorders", {
        query: { limit: 100, offset: i * 100, orderby: "Id desc" },
      ...
      for (const w of page) {
        const t = w.Task?.Id;
        // Newest first, so the first seen for a task is the one in force. Unlike
        // the display index this keeps work orders with no vendor on them: the
        // question here is "does one exist", not "who is on it".
        if (t != null && !byTask.has(t)) byTask.set(t, w.Id);
      }
      if (page.length < 100) break;
    }
    c.byTask = byTask;
  ...
  return c.byTask.get(taskId) ?? null;
}
...
// The single door every live write goes through.
//
// Nothing in this module calls buildiumRequest with a mutating verb directly.
// Routing them all through here means the audit row and the kill-switch check
// cannot be forgotten when a future write is added — they are not something a
// caller opts into.
async function sendWrite({ method, path, payload, taskId, before }) {
  // Checked at the moment of sending, not at module load, so halting takes
  // effect on the very next request rather than the next deploy.
  if (await writesHalted()) {
    const err = new Error("Writing to Buildium is switched off right now. Nothing was changed.");
    err.code = "BUILDIUM_WRITES_HALTED";
    throw err;
  }

  const logId = await beginWrite({ method, path, taskId, payload, before, actor: currentWriteActor() });
  try {
    const result = await buildiumRequest(path, { method, body: payload });
    await finishWrite(logId, { ok: true, status: 200, body: result });
    return result;
  } catch (e) {
    await finishWrite(logId, { ok: false, error: e?.message });
    throw e;
  }
}
...
function invalidateTaskCaches() {
  ...
  if (globalThis.__flWorkOrderScan) globalThis.__flWorkOrderScan = { at: 0, byTask: null };
}
