...
import { buildiumRequest } from "./real.js";
import { isBuildiumLive } from "../env.js";
...
async function buildDirectory() {
  ...
  for (const o of owners) {
    ...
    const propertyIds = (o.PropertyIds || []).filter((n) => n != null);
    // Owners were the only role matched without an id. Residents carried their
    // tenant and lease, vendors their vendor id, and owners carried a name and
    // the words "Portfolio Owner" — which meant nothing about them could be
    // looked up afterwards, including what they are owed.
    const identity = {
      role: "owner",
      entity: {
        name,
        ownerId: o.Id,
        propertyIds,
        sub: propertyIds.length === 1 ? "1 property" : `${propertyIds.length} properties`,
      },
    };
...
//
// By default a directory failure is swallowed and null is returned, because
// matching is best-effort on the login path. With { strict: true } that
// failure is thrown instead: signup uses this, because there "null" would
// mean "not a real person, safe to create", and on an outage that is exactly
// the wrong conclusion. A stale directory is still served rather than thrown,
// since it was correct within the last five minutes.
export async function matchByEmail(email, { strict = false } = {}) {
  ...
  if (!fresh) {
    try { c.byEmail = await buildDirectory(); c.at = Date.now(); }
      ...
      if (strict && !c.byEmail) throw e;
      if (!c.byEmail) return null;
    }
  }
