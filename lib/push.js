...
// A device registered through the App Store / Play Store build.
//
// Web push does not work inside an iOS WebView, so the native app registers with
// APNs or FCM instead and hands us that token. It lands in the same table because
// "which devices does this person have" is one question, distinguished by `kind`.
export async function saveNativeToken(userId, token, platform) {
  if (!isSupabaseConfigured() || tableMissing) return { ok: false, error: "Notifications aren't configured." };
  if (!token || !["ios", "android"].includes(platform)) {
    return { ok: false, error: "That device registration is incomplete." };
  }
  try {
...
export async function sendPush(userId, { title, body, url = "/app", tag }) {
  ...
  try {
    const { data, error } = await getAdminSupabase()
      .from(TABLE).select("endpoint, p256dh, auth, kind").eq("user_id", userId).is("failed_at", null);
  ...
  const dead = [];
  let sent = 0, failed = 0, nativePending = 0;

  // Native devices need APNs (iOS) or FCM (Android), which need credentials that
  // only exist once the Apple and Google developer accounts do. Until then these
  // are counted separately and reported as NOT sent, because a device we cannot
  // reach must never be added to the "we told them" total.
  const web = rows.filter((r) => (r.kind || "web") === "web");
  nativePending = rows.length - web.length;

  await Promise.all(web.map(async (r) => {
    try {
  ...
  return { sent, failed, devices: rows.length, nativePending };
}
